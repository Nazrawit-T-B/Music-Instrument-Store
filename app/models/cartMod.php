<?php
    ...
    class cartModel extends ParentModel {
        ...
        public function addOrUpdate($userId, $productId, $quantity) {
            $stmt = $this->conn->prepare("SELECT quantity FROM cart_items WHERE user_id = ? AND product_id = ?");
            $stmt->execute([$userId, $productId]);
            $existing = $stmt->fetch();

            if ($existing) {
                $newQuantity = $existing['quantity'] + $quantity;
                $update = $this->conn->prepare("UPDATE cart_items SET quantity = ? WHERE user_id = ? AND product_id = ?");
                $update->execute([$newQuantity, $userId, $productId]);
            } else {
                $insert = $this->conn->prepare("INSERT INTO cart_items (user_id, product_id, quantity) VALUES (?, ?, ?)");
